...
max_llf = cmpfun({function(theta, data, lbounds, ubounds, maxIter) {
	
	mll = optim(par = theta, fn = log_llf, data = data, lb = lbounds, ub = ubounds, method = "Nelder-Mead", control = list(fnscale = -1, maxit = maxIter))
...
# return the log likelihood function of the data (scalar)
...
log_llf = cmpfun({function(theta, data, lb, ub) {
  ...
  if ((hit_lb >= 0) && (hit_ub >= 0)) {
    logllf = log_hat_p_K_vec(data[1:(m-1)], Delta, data[2:m], theta)
    logllf = sum(logllf)
  } else {
    logllf = -Inf
...
#lb : lower bound on the parameter
#ub : upper bound on the parameter
log_llf_vec = cmpfun({function(theta, data, lb, ub) {
...
log_hat_p_K_vec = cmpfun({function(v, t, w, theta, PT1 = PT, all_E1 = all_E, all_U1 = all_U, all_V1 = all_V, all_N11 = all_N1, all_N21 = all_N2, all_N31 = all_N3, all_D1 = all_D) {

# v: observations at t
# w: observations at t+1
# t: frequency of the observations
# theta: parameter at which the likelihood function is computed  
  ...
  # log of the function Psi_hat
	log_psi = log_td = matrix(0, nrow = K, ncol = length(y_vec))

  # loop over the MC samples
	for (k in 1:K) {

	    # log_alg_5_1_vec compute log_psi according to formula (15) in the paper
		log_psi[k,] = log_alg_5_1_vec(t, x_vec, y_vec, theta, v[1], PT1[k,], all_E1[k,], all_U1[k,], all_V1[k,], all_N11[k,], all_N21[k,], all_N31[k,], all_D1[k,], maxP)
    	
    	# log_td is the log of the transition density according to formula (12) in the paper
		log_td[k,] = log_psi[k,] + a_alg(y_vec, theta, v[1]) - a_alg(x_vec, theta, v[1]) - log(sigma(w, theta)) - 0.5*log(2*pi)
	}	

  # compute the the expectation of the likelihood function
...
st_errors = cmpfun({function(theta, data, t) {
  ...
  # Compute the gradient of the  log likelihood function, but in vector form
	...
	grad_log_td = numericDeriv(quote(log_hat_p_K_vec(v, t, w, theta)), c("theta"), myenv)
...
#############################################################################################################
# these functions are used to compute the density when v and w are one-dimensional

# Logarithm of the density estimator of Theorem 4.1 when v and w are one-dimensional
log_hat_p_K = cmpfun({function(v, t, w, theta, x0, PT2 = PT, all_E2 = all_E, all_U2 = all_U, all_V2 = all_V, all_N12 = all_N1, all_N22 = all_N2, all_N32 = all_N3, all_D2 = all_D) {
	...
	# log of the function Psi_hat
	log_psi = log_td = rep(0, times = K)
	# loop over the MC samples
	for (k in 1:K) {
	  # log_alg_5_1_vec compute log_psi according to formula (15) in the paper
	  log_psi[k] = log_alg_5_1(t, x, y, theta, x0, PT2[k,], all_E2[k,], all_U2[k,], all_V2[k,], all_N12[k,], all_N22[k,], all_N32[k,], all_D2[k,], maxP)
	  # log_td is the log of the transition density according to formula (12) in the paper
	  log_td[k] = log_psi[k] + a_alg(y, theta, x0) - a_alg(x, theta, x0) - log(sigma(w, theta)) - 0.5*log(2*pi)
	}	
...
opt_ell_L_rho = cmpfun({function(ell_L_rho, Delta, theta0, lbounds, ubounds) {
  
  # Compute min of the max(variance times running time)
  res = optim(ell_L_rho, max_var_rt, t2 = Delta, theta = theta0, lb=lbounds, ub=ubounds, method = "Nelder-Mead")
  
  # returns the value function and the optimal parameters
  c(res$value, res$par)
...
max_var_rt = cmpfun({function(ellLrho, t2, theta, lb, ub) {
  ...
  if ((hit_lb >= 0) && (hit_ub >= 0)) {	
	...
	mvt = max_var_rt_loc(par, t2, PT2, all_E2, all_U2, all_V2, all_N12, all_N22, all_N32, all_D2) 
...
max_var_rt_loc = cmpfun({function(pars, t, PT, all_E, all_U, all_V, all_N1, all_N2, all_N3, all_D) {
  ...
  if ((hit_lb >= 0) && (hit_ub >= 0)) {
    ...
    time = system.time({mlv = max_log_var(w, t, v, theta, PT, all_E, all_U, all_V, all_N1, all_N2, all_N3, all_D)})
...
max_log_var = cmpfun({function(wvec, Delta, vvec, theta, PT2 = PT, all_E2 = all_E, all_U2 = all_U, all_V2 = all_V, all_N12 = all_N1, all_N22 = all_N2, all_N32 = all_N3, all_D2 = all_D) {
  
  var_vec = log_hat_V_vec2(vvec, Delta, wvec, theta, PT2, all_E2, all_U2, all_V2, all_N12, all_N22, all_N32, all_D2)
...
log_hat_V_vec2 = cmpfun({function(v, t, w, theta, PT1 = PT, all_E1 = all_E, all_U1 = all_U, all_V1 = all_V, all_N11 = all_N1, all_N21 = all_N2, all_N31 = all_N3, all_D1 = all_D) {
  ...
  log_psi_square = log_td_square = matrix(0, nrow = K, ncol = length(y_vec))
  for (k in 1:K) {
    log_psi_square[k,] = 2*log_alg_5_1_vec(t, x_vec, y_vec, theta, v[1], PT1[k,], all_E1[k,], all_U1[k,], all_V1[k,], all_N11[k,], all_N21[k,], all_N31[k,], all_D1[k,], maxP)
    log_td_square[k,] = log_psi_square[k,] + 2*a_alg(y_vec, theta, v[1]) - 2*a_alg(x_vec, theta, v[1]) - 2*log(sigma(w, theta)) - log(2*pi)
  }  
